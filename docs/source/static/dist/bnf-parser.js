...
function CompileNot$1(ctx, expr) {
    ...
    return ctx.m.block(outer, [
        ...
        ctx.m.call("_reach_update", [
            ctx.m.i32.add(ctx.m.i32.load(4, 4, ctx.m.local.get(rewind, binaryen.i32)), ctx.m.local.get(count, binaryen.i32))
        ], binaryen.none),
        ...
        */
        expr.count == "+" || expr.count == "1" ?
            ctx.m.if(ctx.m.i32.lt_s(ctx.m.local.get(count, binaryen.i32), ctx.m.i32.const(1)), ctx.m.block(null, [
                // mark failed + rollback ALL progress
                ctx.m.local.set(error, ctx.m.i32.const(1)),
                ctx.m.global.set("index", ctx.m.i32.load(4, 4, ctx.m.local.get(rewind, binaryen.i32))),
                ctx.m.global.set("heap", ctx.m.local.get(rewind, binaryen.i32)),
                ctx.m.br(outer)
            ])) :
            ctx.m.nop(),
...
function GenerateInit(m) {
    m.addFunction("_init", binaryen.none, binaryen.i32, [], m.block(null, [
        ...
        m.global.set("heap", m.call("_roundWord", [
            m.i32.add(m.global.get("inputLength", binaryen.i32), m.global.get("input", binaryen.i32))
        ], binaryen.i32)),
...
function GenerateMatchString(m) {
    m.addFunction("_matchString", binaryen.createType([binaryen.i32, binaryen.i32, binaryen.i32]), binaryen.i32, [
        ...
        m.local.set(0, m.i32.add(m.local.get(0, binaryen.i32), m.global.get("input", binaryen.i32))),
        m.local.set(3, m.i32.const(0)),
        m.block("outer", [
            m.loop("loop", m.block(null, [
                m.br_if("outer", m.i32.ne(m.i32.load8_u(0, 1, m.i32.add(m.local.get(0, binaryen.i32), m.local.get(3, binaryen.i32))), m.i32.load8_u(0, 1, m.i32.add(m.local.get(1, binaryen.i32), m.local.get(3, binaryen.i32))))),
                m.local.set(3, m.i32.add(m.local.get(3, binaryen.i32), m.i32.const(1))),
                m.br_if("outer", m.i32.ge_s(m.local.get(3, binaryen.i32), m.local.get(2, binaryen.i32))),
                m.br_if("outer", m.i32.ge_s(m.i32.add(m.local.get(0, binaryen.i32), m.local.get(3, binaryen.i32)), m.global.get("heap", binaryen.i32))),
                m.br("loop")
            ]))
        ]),
...
function MapBytes2String$1(str, bytes, byteOffset = 0, ref) {
...
function MapTreeRefs$1(tree, str) {
    let stack = [tree];
    let byteOffset = 0;
    let overlap = {
    ...
    while (stack.length > 0) {
        ...
        if (!curr.ref) {
            overlap = overlap.bytes === curr.start ? overlap :
                MapBytes2String$1(str, curr.start, byteOffset, overlap.ref);
            curr.ref = new ReferenceRange$1(overlap.ref.clone(), new Reference$1(0, 0, 0));
            ...
            overlap = overlap.bytes === curr.end ? overlap :
                MapBytes2String$1(str, curr.end, byteOffset, overlap.ref);
...
function Parse$1(ctx, data, refMapping = true, entry = "program") {
    ...
    if (statusCode == 1) {
        if (refMapping) {
            return new ParseError$1("Unable to parse", new ReferenceRange$1(new Reference$1(0, 0, 0), MapBytes2String$1(data, reach, 0, Reference$1.blank()).ref));
        }
        else {
            return new ParseError$1("Unable to parse", new ReferenceRange$1(new Reference$1(0, 0, 0), MapBytes2String$1(data, reach, 0, Reference$1.blank()).ref));
        }
    }
    const root = Decode$1(ctx, heap, refMapping);
    if (refMapping) {
        MapTreeRefs$1(root, data);
    }
    return {
        root,
        reachBytes: reach,
        reach: refMapping ?
            MapBytes2String$1(data, reach, root.end, ((_a = root.ref) === null || _a === void 0 ? void 0 : _a.end.clone()) || Reference$1.blank()).ref :
            null,
        isPartial: root.end < ctx.exports.inputLength.value
    };
}
function Decode$1(ctx, heap, readBoundary = false) {
    const memory = ctx.exports.memory;
...
function toString() {
    return `import "./shared.js";\n` +
        `const OFFSET = ${JSON.stringify(OFFSET$1)};` +
        "\nexport " + InitParse$1.toString() +
        "\nexport " + MapBytes2String$1.toString() +
...
function MapBytes2String(str, bytes, byteOffset = 0, ref) {
    const encoder = new TextEncoder();
    // const ref = from.clone();
    while (byteOffset <= bytes && ref.index < str.length) {
        const char = str[ref.index];
...
function Decode(ctx, heap, readBoundary = false) {
    const memory = ctx.exports.memory;
    const memoryArray = new Int32Array(memory.buffer);
    const byteArray = new Int8Array(memory.buffer);
    const decoder = new TextDecoder();
...
let _rawWasm = DecodeBase64("");
let _ctx = null;
...
export { shared as Shared, index$1 as legacy, types, index as wasm };
